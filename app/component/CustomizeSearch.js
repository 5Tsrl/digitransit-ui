...
state= { this.context.location.query.accessibilityOption === '1'}
      */}
    <h4>{this.context.intl.formatMessage({
        id: 'accessibility',
        defaultMessage: 'Accessibility',
      })}</h4>
    <ToggleButton
        ...
        key='accessibility-button'
        icon='wheelchair'
        className={cx('small-4','first-btn', 'last-btn')}
        state={this.getAccessibilityOption()}
        checkedClass='bus'
        label='accessibility'
        onBtnClick={() => this.toggleAccessibilityOption()}
      />
    </section>
  );
  ...
  if (getCustomizedSettings().modes) {
      return getCustomizedSettings().modes;
    }
    return getDefaultTransportModes(this.context.config);
  }
  ...
  toggleAccessibilityOption() {
    const accessibilityOn = this.context.location.query.accessibilityOption === '1'
    this.context.router.replace({
      ...this.context.location,
      query: {
        ...this.context.location.query,
        accessibilityOption: ( accessibilityOn ? '0' : '1'),
        modes:  (!accessibilityOn ? without(  this.getModes(),'CITYBIKE').join(',')   :  this.getModes().join(',') )
      },
    });
